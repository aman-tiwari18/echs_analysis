"""
Module 18 — Pattern 06: Untouched High-Billers
Hospitals with high mean bills, zero deduction, zero rejection — never scrutinized.
"""
...
query = """
    SELECT
        ...
        ROUND((SUM(cs.CS_NET_CLAIM_AMT) - SUM(cs.CS_UTI_APP_AMT)) / NULLIF(SUM(cs.CS_NET_CLAIM_AMT),0) * 100, 2) as deduction_pct,
    ...
    HAVING COUNT(*) >= 100
      AND mean_bill >= 100000
      AND (deduction_pct IS NULL OR deduction_pct <= 0.5)
...
def run_query():
    ...
    try:
        ...
        print("Executing Pattern 06 (Untouched High-Billers)...")
        ...
        if len(lines) > 1:
            ...
            fn = os.path.join(data_dir, f'06_Untouched_High_Billers_{ts}.csv')
